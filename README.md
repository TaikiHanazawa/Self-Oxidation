...
impactor 質量・速度
  → melt pool 深さ
  → melt pool 底圧力 P_eq
  → ΔIW_eq ≈ −2.0（金属-ケイ酸塩平衡条件，Section 3-6A）→ D_N, D_H, D_C へ渡す
...
#### (A) ΔIW_eq：金属-ケイ酸塩平衡条件下（D_N, D_H, D_C の入力）
...
a_FeO^silicate = γ_FeO × X_FeO
  γ_FeO = 1.55  （X_FeO > 0.1）
  γ_FeO = 1.8–2.0  （X_FeO < 0.09）
a_Fe^alloy = 0.75  （PtFe 合金実験値の代表値）
```

Zhang 2024 の数値結果（Komabayashi 2014 EOS を使った計算結果）：
- high-T adiabat（T₀ = 2500 K）: ΔIW_eq = −2.25 → −2.11（0→80 GPa）
- low-T adiabat（T₀ = 2100 K）: ΔIW_eq = −2.10 → −1.90（0→80 GPa）
- → **ΔIW_eq ≈ −2 はほぼ圧力に依存しない**（全 GI 範囲で実質定数）

実装上の扱い：
- **ΔIW_eq = −2.0**（固定値として採用；Zhang 2024 の ±0.15 の変動は無視）
- → D_N（Shi 2022 Eq.1, 2），D_H（Tsutsumi 2025 Eq.3），D_C（Fischer 2020 Eq.1）に渡す
